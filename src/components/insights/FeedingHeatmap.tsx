  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/30" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        <span className="bg-blue-100 text-blue-700 text-sm px-3 py-1.5 rounded-full font-medium inline-block">
          {count} {t("feedingSessions")}
        </span>
      </div>
    </div>
  );
        ...
        <svg width={width} height={svgH}>
          ...
          {days.map(({ label, idx }) => (
            <text
              key={idx}
              x={MARGIN_LEFT - 4}
              y={MARGIN_TOP + idx * cellH + cellH / 2}
              textAnchor="end"
            ...
            Array.from({ length: HOUR_BUCKETS }, (_, b) => {
              ...
              return (
                ...
                >
                  ...
                  {count > 0 && (
                    <text x={x + w / 2} y={y + h / 2} textAnchor="middle" dominantBaseline="middle" fontSize={8} fill={count / maxCount > 0.5 ? "white" : "#3b82f6"} fontWeight={700}>
                      {count}
                    </text>
                  )}
                </g>
          ...
          {!hasData && (
            <text x={MARGIN_LEFT + plotW / 2} y={MARGIN_TOP + 3.5 * cellH} textAnchor="middle" dominantBaseline="middle" fontSize={12} fill="#d1d5db">
              {t("noFeedingData")}
            </text>
          )}
        </svg>
